...
public class setLivesCommand implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if(strings.length < 2){
            Msg.send(commandSender, "&rUsage: /set_lives <player> <amount>");
            return false;
        }
        ...
        if(player.hasPermission("hardcore.setLives") || player.isOp()){
            ...
            hcp.setLives(Integer.parseInt(strings[1]));
            ...
            Msg.send(player, "&rYou do not have permission to use this command");
        }
